...
token = "MTAyNDI1ODY0MjI2ODE0MzY2Ng.GckQ.LPWs7iPvMagDG3SmhMd4VPgI5bc7komrVwlkyE"
...
fenixplayz_id = "808230072536006697"
client_id = "1024259879696859146"
supportlink = "https://discord.gg/introverts"
invitelink = f"https://discordapp.com/oauth2/authorize?client_id={client_id}&scope=client+applications.commands&permissions=8"
...
async def boosts(ctx):
  ...
  boost.add_field(name=f"<a:boost:1039500698494177310> Boost Info For: {ctx.guild.name} <a:boost:1039500698494177310>", value=f"\n▸ Total Boost: {ctx.guild.premium_subscription_count}\n▸ Boost Level: {ctx.guild.premium_tier}\n")
...
async def credit(ctx):
    embed = discord.Embed(
        ...
        f'Xeone Credits',
        description=
        f'Thanks To Fenix, Hacker, Anant, OAuth, XmokePlayZ For Help With The Bot!\n',
        color=0x2f3136)
    embed.set_footer(text="")
    await ctx.send(embed=embed)
...
async def invite(ctx):
    embed = discord.Embed(
        color=0x2f3136,
        description=
        f"**[Get Xeone](https://discord.com/api/oauth2/authorize?client_id={client_id}&permissions=8&scope=bot) \n[Support Server](https://discord.gg/introverts)**"
    )
...
async def about(ctx):
    embed = discord.Embed(
        color=0x2f3136,
        description=
        f"Xeone Is A Multipuropose Bot Which Supports More Than 10+ Category And 80+ Commands Specifically Made For Managing Your Server In An Unique Way."
    )
    await ctx.send(embed=embed, mention_author=True)
...
async def membercount(ctx):
    ...
    scembed.add_field(
        name='**<:fenix_dash:1007475039731466320> Members**',
...
async def roleall(ctx, role: discord.Role):
  ...
  if role.permissions.administrator or role.permissions.ban_members:
    await ctx.send(embed=discord.Embed(color=discord.Colour(0x2f3136),title=f"Xeone",description=f"Use A Role Without Dangerous Perms Like: `Administartors`, `Ban Members` & Much More!"))
  else:
    mm = await ctx.send(embed=discord.Embed(color=discord.Colour(0x2f3136),title=f"Xeone",description=f"<a:jhebwbsajabhJa:1024918925319872522> Adding Mentioned Role To @everyone"))
    for all in ctx.guild.members:
      try:
        await all.add_roles(rrole)
      except:
        pass
    await mm.edit(embed=discord.Embed(color=discord.Colour(0x2f3136),title=f"Xeone",description=f"Added Mentioned Role To @everyone"))
...
async def botinfo(ctx):
    embed = discord.Embed(
        title=
        f'Xeone Stats N Info',
        description=
        f'**__Server(s)__\n```\nTotal Server(s): {len(client.guilds)}\nTotal Shard(s): 2\nShard(s) Server: {len(client.guilds)}```\n__Version Info__\n```\nPython: 3.8.12\nLibrary: Discord.py\nLibrary Version: V2```\n__HostInfo__```\nHost: LD-Linux x86\nLatency: {int(client.latency * 1000)}\nWesocket: {int(client.latency * 750)}\nReply Latency: {int(client.latency * 400)}```\n```Prefix: !```\n\n__Developer__\n```Fenix#1337```\n__Owners__\n```Criminal, Xmoke, OAuth```\n\n__Link(s)__\n[Admin Invite](https://discordapp.com/oauth2/authorize?client_id=1024259879696859146&scope=bot+applications.commands&permissions=8) | [0 Perms Invite](https://discordapp.com/oauth2/authorize?client_id=1024259879696859146&scope=bot+applications.commands&permissions=0)\n[Support](https://discord.gg/k752PRrcWG) | [Vote Soon](https://discordapp.com/oauth2/authorize?client_id=1024259879696859146&scope=bot+applications.commands&permissions=8)**',
        color=0x2f3136)
    embed.set_footer(text="Made With 🧠 By Fenix#1337")
    await ctx.send(embed=embed)
...
async def meme(ctx):
    ...
    embed.set_footer(text=f"Memes | Xeone", icon_url = ctx.author.avatar_url)
    await ctx.send(embed=embed)

@client.command()
async def nitro(ctx):
  em = discord.Embed(color=discord.Colour(0x2f3136), title="Xeone", description="**You Have Been Rick-Rolled xD! 🤣**")
...
async def qr(ctx, input):
    if input == None:
        ...
        embed = discord.Embed(title="Xeone | QR Generated")
        embed.set_image(
            url=
...
@client.command(aliases=["nsf"])
async def nsfw(ctx):
    embed = discord.Embed(
        title=
        f'Xeone - NSFW Commands',
        description=
        f'`nsfw4k` | `boobs` | `pussy` | `lewd` | `lesbian` | `spank` | `cum`',
        color=0x2f3136)
    embed.set_footer(text="")
    await ctx.send(embed=embed)
...
@client.command(aliases=["funs"])
async def fun(ctx):
    embed = discord.Embed(
        title=
        f'Xeone - Fun Commands',
        description=
        f'`qr` | `meme` | `truth` | `dare` | `hack [user]` | `gay [user]` | `gender [user]`',
        color=0x2f3136)
    embed.set_footer(text="")
    await ctx.send(embed=embed)
